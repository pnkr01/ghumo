...
class CarouselHotel extends StatefulWidget {
  const CarouselHotel({Key? key}) : super(key: key);

  @override
  State<CarouselHotel> createState() => _CarouselHotelState();
}

class _CarouselHotelState extends State<CarouselHotel> {
  int activeIndex = 0;
  int indexing = 0;
  Stream? slides;
  List? slideList;
  _querydb() {
    ...
    slides = FirebaseFirestore.instance
        .collection('hotel_carousel')
        .doc('ZUp72utMm4wMBi1t6crK')
        .collection(dham)
        .snapshots()
        .map(
          (list) => list.docs.map(
